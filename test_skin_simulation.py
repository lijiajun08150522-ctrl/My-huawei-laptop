...
def simulate_api_response():
    ...
    # ==================== 场景 4: 获取图片 ====================
    print_section("场景 4: 获取生成的图片")
    
    print("📤 请求:")
    print(f"  GET /api/skin/image/{task_id}.png")
    print(f"  Authorization: Bearer {player_id}\n")
    
    print("📥 响应 (200 OK):")
    print(f"  Content-Type: image/png")
    print(f"  Content-Length: 245,678 bytes")
    print(f"  [二进制 PNG 图片数据]")
    
    # ==================== 场景 5: 应用皮肤到游戏 ====================
    print_section("场景 5: 应用皮肤到游戏")
    
    print("📤 请求:")
    print(f"  POST /api/skin/apply")
    print(f"  Content-Type: application/json")
    print(f"  {{")
    print(f'    "skin_id": "{task_id}",')
    print(f'    "player_id": "{player_id}"')
    print(f"  }}\n")
    ...
    # ==================== 场景 7: 错误场景 - prompt 为空 ====================
    print_section("场景 7: 错误处理 - prompt 为空")
    ...
    # ==================== 场景 8: 错误场景 - prompt 超长 ====================
    print_section("场景 8: 错误处理 - prompt 超过长度限制")
    
    long_prompt = "a" * 501  # 501 字符
    ...
    error_response = {
        ...
        "errors": [
            {
                "field": "prompt",
                "message": "prompt 长度不能超过500字符（当前：501）"
            }
        ],
        "error_code": "VALIDATION_ERROR"
    }
    ...
    # ==================== 场景 9: 错误场景 - ComfyUI 服务不可用 ====================
    print_section("场景 9: 错误处理 - ComfyUI 服务不可用")
    ...
    # ==================== 场景 10: 错误场景 - 任务不存在 ====================
    print_section("场景 10: 错误处理 - 任务不存在")
    
    fake_task_id = "task_not_exists"
    
    print("📤 请求:")
    print(f"  GET /api/skin/status/{fake_task_id}\n")
    
    print("📥 响应 (404 Not Found):")
    error_response = {
        "success": False,
        "message": "任务不存在",
        "error_code": "TASK_NOT_FOUND",
        "details": f"找不到任务ID: {fake_task_id}"
    }
    print(json.dumps(error_response, indent=2, ensure_ascii=False))
    
    # ==================== 场景 11: 多个并发生成任务 ====================
    print_section("场景 11: 多个并发生成任务")
    ...
    # ==================== 场景 12: 不同风格的皮肤生成 ====================
    print_section("场景 12: 不同风格的皮肤生成")
    
    styles = ["cartoon", "realistic", "pixel_art"]
    style_prompts = {
        "cartoon": "a cute cartoon snake with big eyes",
        "realistic": "a realistic snake with detailed scales",
        "pixel_art": "a pixel art snake character"
    }
    
    for style in styles:
        print(f"\n风格: {style}")
        print(f"Prompt: {style_prompts[style]}")
        
        task_id = f"task_{int(time.time())}_{style}"
...
def print_summary():
    ...
    print("  5. ✅ 应用皮肤到游戏")
    print("  6. ✅ 获取历史记录")
    print("  7. ✅ 错误处理 - prompt 为空")
    print("  8. ✅ 错误处理 - prompt 超长")
    print("  9. ✅ 错误处理 - ComfyUI 服务不可用")
    print(" 10. ✅ 错误处理 - 任务不存在")
    print(" 11. ✅ 多个并发生成任务")
    print(" 12. ✅ 不同风格的皮肤生成")
    ...
    print("\n🎯 核心功能:")
    print("  ✅ ComfyUI API 集成（通过 prompt 参数）")
    print("  ✅ 异步任务管理")
    print("  ✅ 状态查询和进度跟踪")
    print("  ✅ 图片存储和 URL 返回")
    print("  ✅ 错误处理和验证")
    ...
    print("  - 测试脚本: test_skin_simulation.py")
